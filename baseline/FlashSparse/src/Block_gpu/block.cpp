...
//row_ptr,col_index,row(补齐),nnz,8,8)
...
preprocess_gpu_fs( torch::Tensor nodePointer_tensor, torch::Tensor edgeList_tensor,
  ...
  int window_num = num_nodes/blockSize_h;//TC block的行数
  // auto blockPartition_tensor = torch::zeros({window_num}, torch::kInt32).to(torch::kCPU);
  auto row_window_offset_tensor = torch::zeros({window_num+1}, torch::kInt32).to(torch::kCPU);//一维张量,长度TC行数 + 1// ,数值0
  ...
  auto seg_out_tensor = torch::zeros({edgeList_tensor.size(0)}, torch::kInt32).to(torch::kCPU);//一维张量,长度nnz// ,数值0

  auto edgeList = edgeList_tensor.data<int>();//CSR col_Ind
  // auto blockPartition = blockPartition_tensor.data<int>();
  auto row_window_offset = row_window_offset_tensor.data<int>();//TC block (row_num + 1)
  // auto edgeToColumn = edgeToColumn_tensor.data<int>();
  // auto edgeToRow = edgeToRow_tensor.data<int>();
  auto nodePointer = nodePointer_tensor.data<int>();//CSR row_ptr
  auto seg_out = seg_out_tensor.data<int>();//一维张量,长度nnz// ,数值0

int *edgeList_d, *blockPartition_d, *row_window_offset_d, *edgeToColumn_d, *edgeToRow_d, *nodePointer_d, *seg_out_d;

// 分配 GPU 内存
cudaMalloc(&edgeList_d, (edgeList_tensor.size(0)) * sizeof(int));//CSR col_Ind_GPU
cudaMalloc(&blockPartition_d, (window_num) * sizeof(int));//TC block的行数
cudaMalloc(&row_window_offset_d, (window_num+1) * sizeof(int));//TC block的行数 + 1
cudaMalloc(&edgeToColumn_d, (num_edges) * sizeof(int));//nnz
cudaMalloc(&edgeToRow_d, (num_edges) * sizeof(int));//nnz
cudaMalloc(&nodePointer_d, (nodePointer_tensor.size(0)) * sizeof(int));//CSR row_ptr
cudaMalloc(&seg_out_d, (seg_out_tensor.size(0)) * sizeof(int));//一维张量,长度nnz// ,数值0
  ...
  //#NNZ
  auto blocknum = torch::zeros({1}, options_gpu);//一维张量,长度1,数值0
  auto vectornum = torch::zeros({1}, options_gpu);//一维张量,长度1,数值0
  auto block_num = blocknum.data<int>();//一维张量,长度1,数值0
  auto vector_num = vectornum.data<int>();//一维张量,长度1,数值0
  auto start = std::chrono::high_resolution_clock::now();
  fill_edgeToRow_cuda(edgeToRow_d, nodePointer_d, num_nodes);//nnz长度,CSR row_ptr, 补齐A's row
  //输出edgeToRow_d表示非零元会在哪个warp上执行
  int block_counter = 0;

  fill_segment_cuda(nodePointer_d, seg_out_d, blockSize_h, blockSize_w, num_nodes);
  //CSR row_ptr,一维张量长度nnz数值0,8,8,补齐A's row
  //输出seg_out_d表示非零元会在那个block上执行
  ...
  //seg_out_d,CSR col_Ind,CSR row_ptr,nnz长度,edgeToRow_d,长度TC block的行数,一维张量长度1数值0,一维张量长度1数值0,
  //长度为TC block的行数 + 1,8,8,nnz,nnz,TC block的行数
